...
do
  ...
  echo "===== Disk usage on $ip (port=$port, user=$user) ====="

  # 通过指定密钥 (-i) 和端口 (-p) 进行 SSH 登录并执行命令
  ssh -o StrictHostKeyChecking=no -n -i "$KEY_FILE" -p "$port" "$user@$ip" "df -h --output=source,size,used,avail,pcent,target | grep /dev" | column -t
