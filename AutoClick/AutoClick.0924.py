...
DEFAULT_HOTKEYS = [f"ctrl+shift+{i if i < 8 else 0}" for i in range(1, NUM_POINTS + 1)]
...
class AreaSelectionOverlay:
    """
    全屏半透明蒙版拖拽框选拾取器 (支持矩形和圆形拖拽 preview 预览与坐标计算)
    """
    def __init__(self, parent_root, shape_type="rect", callback=None):
        self.parent_root = parent_root
        self.shape_type = str(shape_type).lower()  # "rect" or "circle"
        ...
        self.draw_shape_id = None
        self.draw_text_id = None

        shape_name = "矩形范围" if self.shape_type == "rect" else "圆形范围"
        self.canvas.create_text(
            self.top.winfo_screenwidth() // 2,
            40,
            text=f"【{shape_name} 拖拽框选拾取】在屏幕/模拟器上按住鼠标左键拖拽框选范围，按 Esc 键取消",
            fill="#f39c12",
    ...
    def on_move_press(self, event):
        ...
        if self.draw_shape_id:
            self.canvas.delete(self.draw_shape_id)
        if self.draw_text_id:
            self.canvas.delete(self.draw_text_id)
        ...
        if self.shape_type == "rect":
            self.draw_shape_id = self.canvas.create_rectangle(x1, y1, x2, y2, outline="#FF0000", width=3)
            self.draw_text_id = self.canvas.create_text(
                x1 + w // 2, max(25, y1 - 15),
                text=f"矩形范围: ({x1},{y1}) -> ({x2},{y2}) [宽{w} x 高{h}]", fill="#FF0000", font=("Segoe UI", 12, "bold")
            )
        else:
            cx, cy = (self.start_x + self.current_x) // 2, (self.start_y + self.current_y) // 2
            r = int(math.hypot(self.current_x - self.start_x, self.current_y - self.start_y) / 2.0)
            self.draw_shape_id = self.canvas.create_oval(cx - r, cy - r, cx + r, cy + r, outline="#FF0000", width=3)
            self.draw_text_id = self.canvas.create_text(
                cx, max(25, cy - r - 15),
                text=f"圆形范围: 圆心({cx},{cy}), 半径={r}px", fill="#FF0000", font=("Segoe UI", 12, "bold")
            )
...
def macro_int(val=0):
    """安全转换为整数 (支持 int, float, bool, 字符串及 NaN/None 容错)"""
    if val is None:
        return 0
...
def macro_mod(a=0, b=1):
    """安全取余/求模 (支持 mod(a, b) 函数及 a mod b 运算符)"""
    try:
        fa = float(a)
        fb = float(b)
        if fb == 0:
        ...
        try:
            ia, ib = int(a), int(b)
            return ia % ib if ib != 0 else 0
        except Exception:
            return 0


def _prepare_macro_eval_expr(expr_str, scope_vars):
    """预处理脚本宏表达式/条件字符串并构建安全的 eval 环境"""
    expr = str(expr_str).strip()

    # 清除 $ 前缀变量标识 (如 $r -> r)
    expr_trans = re.sub(r"\$([a-zA-Z_]\w*)", r"\1", expr)

    # 替换大小写关键词
    expr_trans = re.sub(r"\btrue\b", "True", expr_trans, flags=re.IGNORECASE)
    ...
    expr_trans = re.sub(r"\bint\b(?=\s*\()", "int", expr_trans, flags=re.IGNORECASE)
    expr_trans = re.sub(r"\bmod\b(?=\s*\()", "mod", expr_trans, flags=re.IGNORECASE)

    # 支持 a mod b 中缀运算符 -> a % b
    expr_trans = re.sub(r"\bmod\b(?!\s*\()", " % ", expr_trans, flags=re.IGNORECASE)
    ...
    return expr, expr_trans, safe_env


def safe_eval_expr(expr_str, scope_vars):
    """安全求值表达式/变量值 (支持 int, mod, rand, float, bool, str, 算术运算及变量查找)"""
    expr, expr_trans, safe_env = _prepare_macro_eval_expr(expr_str, scope_vars)
    ...
    try:
        ...
        if clean_expr in safe_env:
            return safe_env[clean_expr]
        return expr


def safe_eval_cond(cond_str, scope_vars):
    """安全求值 If 条件表达式 (支持 ==, !=, >, <, >=, <=, mod, int, and, or, not, 单变量真值判断)"""
    ...
    _, cond_trans, safe_env = _prepare_macro_eval_expr(cond, scope_vars)

    try:
        return bool(eval(cond_trans, safe_env))
    except Exception:
        clean_cond = cond.lstrip("$")
        if clean_cond in safe_env:
            return bool(safe_env[clean_cond])
        return False


def parse_location_spec(loc_str):
    """
    解析位置/坐标规范表达式，支持:
    1. 矩形范围: rect(x1, y1, x2, y2)
    2. 圆形范围: circle(x, y, r)
    3. 基础点坐标: x, y 或 (x, y) 或 x
    返回字典结构: {"type": "rect"|"circle"|"point", ...}
    """
    s = str(loc_str).strip()
    m_rect = re.match(r"^rect\s*\(\s*([^,]+)\s*,\s*([^,]+)\s*,\s*([^,]+)\s*,\s*([^,]+)\s*\)$", s, re.IGNORECASE)
    if m_rect:
        return {
            "type": "rect",
            "x1": m_rect.group(1).strip(),
            "y1": m_rect.group(2).strip(),
            "x2": m_rect.group(3).strip(),
            "y2": m_rect.group(4).strip(),
            "raw": s,
        }
    m_circle = re.match(r"^circle\s*\(\s*([^,]+)\s*,\s*([^,]+)\s*,\s*([^,]+)\s*\)$", s, re.IGNORECASE)
    if m_circle:
        return {
            "type": "circle",
            "x": m_circle.group(1).strip(),
            "y": m_circle.group(2).strip(),
            "r": m_circle.group(3).strip(),
            "raw": s,
        }
...
def sample_random_location(loc_str_or_spec, scope_vars=None):
    """
    对 location 表达 (rect / circle / point) 求值并在其区域内部进行高斯离散散布采样，
    返回 (x, y) 整数坐标元组。
    """
    if scope_vars is None:
        scope_vars = {}

    if isinstance(loc_str_or_spec, dict):
        spec = loc_str_or_spec
    else:
        spec = parse_location_spec(loc_str_or_spec)

    t = spec.get("type", "point")

    def _eval_num(expr_val, default_v=0.0):
        try:
            res = safe_eval_expr(str(expr_val), scope_vars)
            return float(res)
        except Exception:
            try:
    ...
    if t == "rect":
        x1 = _eval_num(spec["x1"], 0)
        y1 = _eval_num(spec["y1"], 0)
        x2 = _eval_num(spec["x2"], 0)
        y2 = _eval_num(spec["y2"], 0)
        ...
        if w > 0:
            dx = random.gauss(0, w / 2.0)
            dx = max(-w, min(w, dx))
        else:
            dx = 0.0

        if h > 0:
            dy = random.gauss(0, h / 2.0)
            dy = max(-h, min(h, dy))
        else:
            dy = 0.0
        ...
        cx = _eval_num(spec["x"], 0)
        cy = _eval_num(spec["y"], 0)
        r = abs(_eval_num(spec["r"], 0))

        if r > 0:
            angle = random.uniform(0, 2 * math.pi)
            dist = abs(random.gauss(0, r / 2.0))
            dist = min(r, dist)
            dx = dist * math.cos(angle)
            dy = dist * math.sin(angle)
            final_x = int(round(cx + dx))
            final_y = int(round(cy + dy))
            while math.hypot(final_x - cx, final_y - cy) > r:
                scale = 0.95
                final_x = int(round(cx + (final_x - cx) * scale))
                final_y = int(round(cy + (final_y - cy) * scale))
            return final_x, final_y
        else:
            return int(round(cx)), int(round(cy))

    else:
        x = _eval_num(spec.get("x", 0), 0)
        y = _eval_num(spec.get("y", 0), 0)
        return int(round(x)), int(round(y))


def eval_delay_or_interval_expr(expr_str, scope_vars=None):
    """求值延迟/间隔表达式，支持 "1,3" 区间字符串、rand(1,3) 以及常规数值"""
    if scope_vars is None:
        scope_vars = {}
    s = str(expr_str).strip()
    if not s:
        return 0.0
    if "," in s and not s.lower().startswith("rand") and not s.lower().startswith("random") and "(" not in s:
        parts = [p.strip() for p in s.split(",") if p.strip()]
        if len(parts) >= 2:
            return float(macro_rand(parts[0], parts[1]))
    try:
        return float(safe_eval_expr(s, scope_vars))
    except Exception:
        return 0.0
...
def parse_macro_script(script_text):
    ...
    for idx, line in enumerate(lines, 1):
        ...
        # ClickEx(level, x, y, delay, interval, count, "remark", timer_id) 或 ClickEx(level, rect/circle, delay, ...)
        ...
        if m_cex:
            ...
            if len(parts) >= 2 and (parts[1].lower().startswith("rect(") or parts[1].lower().startswith("circle(")):
                level_expr = parts[0]
                x_expr = parts[1]
                y_expr = "0"
                if len(parts) >= 7:
                    delay_expr, interval_expr, count_expr = parts[2], parts[3], parts[4]
                    remark_expr, timer_id_expr = parts[5], parts[6]
                elif len(parts) == 6:
                    delay_expr, interval_expr, count_expr = parts[2], parts[3], parts[4]
                    remark_expr = parts[5]
                elif len(parts) == 5:
                    ...
                    delay_expr, interval_expr = parts[2], parts[3]
                elif len(parts) == 3:
                    delay_expr = parts[2]
            else:
                if len(parts) >= 8:
        ...
        # Click(x, y) 或 Click(rect/circle) 或 Click(x, y, count) 或 Click(x, y, count, interval_ms)
        m_click = re.match(r"^Click\s*\(\s*(.*?)\s*\)$", raw, re.IGNORECASE)
        if m_click:
            parts = split_macro_args(m_click.group(1))
            if len(parts) >= 1 and (parts[0].lower().startswith("rect(") or parts[0].lower().startswith("circle(")):
                x_expr = parts[0]
                y_expr = "0"
                cnt_expr = parts[1] if len(parts) >= 2 else "1"
                intv_expr = parts[2] if len(parts) >= 3 else "100"
            else:
...
def detect_adb_ports_by_hwnd(hwnd):
    ...
    try:
        ...
        root_h = win32gui.GetAncestor(hwnd, win32con.GA_ROOT) or hwnd
        _, root_pid = win32process.GetWindowThreadProcessId(root_h)

        target_pids = set()
        if pid:
            target_pids.add(str(pid))
        if root_pid:
            target_pids.add(str(root_pid))
            ...
            if len(parts) >= 5 and parts[0].upper().startswith("TCP") and parts[3].upper() == "LISTENING":
                ...
                if proc_pid in target_pids:
                    ...
                    if ":" in local_addr:
                        ...
                        try:
                            port = int(port_str)
                            if port not in found_ports and 1000 <= port <= 65535:
...
class AutoClickerApp:
    def __init__(self, root):
        ...
        # 模拟器窗口尺寸与双轨自适应基准 (自适应缩放与一键还原，从持久化配置中加载)
        session_data = get_session_state()
        init_win_size = session_data.get("standard_window_size")
        if init_win_size and isinstance(init_win_size, (list, tuple)) and len(init_win_size) >= 2 and init_win_size[0] and init_win_size[1]:
            self.target_window_size = [int(init_win_size[0]), int(init_win_size[1])]
        else:
        ...
        for i in range(NUM_POINTS):
            p_dict = {
                ...
                "count": tk.StringVar(value="0"),  # 点击次数 (0表示无限次)
                "hotkey": tk.StringVar(value=DEFAULT_HOTKEYS[i] if i < len(DEFAULT_HOTKEYS) else f"ctrl+shift+{i+1}"),
            }
        ...
        self.points_collapsed = False
        self.editor_collapsed = False
        self.points_sash_pos = 435
        self.script_sash_pos = 435
    ...
    def format_log_msg_seconds(self, msg):
        """对日志内容部分中以秒为单位的时间数字统一格式化为 1 位小数 (如 0.5s, 1.0s, 0.5 秒, 1.0 秒)"""
        if not msg:
            return msg
        pattern = r'(?<![a-zA-Z0-9_\.])(\d+(?:\.\d+)?)\s*(s|秒)(?![a-zA-Z0-9_])'

        def repl(m):
            num_str = m.group(1)
            unit = m.group(2)
            start = m.start()
            if start > 0 and msg[start - 1].lower() == 'm':
                return m.group(0)
            val = float(num_str)
            space = " " if " " in m.group(0) else ""
            return f"{val:.1f}{space}{unit}"
    ...
    def log_msg(self, msg):
        ...
        formatted_msg = self.format_log_msg_seconds(str(msg))

        if elapsed_sec is not None:
            full_line = f"[{timestamp} | {elapsed_sec}s{timer_part}] {formatted_msg}\n"
        else:
            full_line = f"[{timestamp}] {formatted_msg}\n"
    ...
    DEFAULT_SASH_POS = 435

    def get_tab_sash_pos(self, tab_widget):
        """动态计算 Tab 面板的精确渲染高度 (包含 26px 选项卡头部)"""
        if not hasattr(self, "root") or not tab_widget:
            return self.DEFAULT_SASH_POS
        self.root.update_idletasks()
        try:
            req_h = tab_widget.winfo_reqheight()
            return max(150, req_h + 26)
        except Exception:
            return self.DEFAULT_SASH_POS

    def set_sash_pos_safe(self, pos):
        """安全设定 PanedWindow 的 sashpos 分割位置 (防未映射窗口归零)"""
        if hasattr(self, "main_paned"):
            try:
                if len(self.main_paned.panes()) >= 2:
                    if self.main_paned.winfo_height() > 50:
                        self.main_paned.sashpos(0, pos)
                    else:
                        self.main_paned.bind("<Map>", lambda e: self.root.after(50, lambda: self.set_sash_pos_safe(pos)), add="+")
            except Exception:
                pass

    def _on_tab_changed(self, event=None):
        """当选项卡在 Point Mode 与 Scripting Mode 之间切换时，恢复各自独立的已选 sash 位置与折叠状态"""
        if not hasattr(self, "notebook"):
            return
        try:
            tab_idx = self.notebook.index(self.notebook.select())
            if tab_idx == 0 and hasattr(self, "tab_points"):
                pos = self.get_tab_sash_pos(self.tab_points)
                self.points_sash_pos = pos
                self.set_sash_pos_safe(pos)
            elif tab_idx == 1 and hasattr(self, "tab_script"):
                pos = self.get_tab_sash_pos(self.tab_script)
                self.script_sash_pos = pos
                self.set_sash_pos_safe(pos)
        except Exception:
            pass

    def toggle_points_collapse(self):
        """折叠/展开 8 组坐标点详细配置表格"""
        if getattr(self, "points_collapsed", False):
            if hasattr(self, "points_header_frame"):
                self.points_header_frame.pack(fill="x", pady=2)
            if hasattr(self, "rows_container"):
                self.rows_container.pack(fill="x", expand=False)
            if hasattr(self, "btn_collapse_points"):
                self.btn_collapse_points.config(text="▲ 折叠配置")
            self.points_collapsed = False
        else:
            if hasattr(self, "rows_container"):
                self.rows_container.pack_forget()
            if hasattr(self, "points_header_frame"):
                self.points_header_frame.pack_forget()
            if hasattr(self, "btn_collapse_points"):
                self.btn_collapse_points.config(text="▼ 展开配置")
            self.points_collapsed = True

        if hasattr(self, "tab_points"):
            self.points_sash_pos = self.get_tab_sash_pos(self.tab_points)
            self.set_sash_pos_safe(self.points_sash_pos)

    def toggle_editor_collapse(self):
        """折叠/展开 按键脚本代码编辑器"""
        if getattr(self, "editor_collapsed", False):
            if hasattr(self, "editor_container"):
                self.editor_container.pack(fill="both", expand=True)
            if hasattr(self, "btn_collapse_editor"):
                self.btn_collapse_editor.config(text="▲ 折叠编辑器")
            self.editor_collapsed = False
        else:
            if hasattr(self, "editor_container"):
                self.editor_container.pack_forget()
            if hasattr(self, "btn_collapse_editor"):
                self.btn_collapse_editor.config(text="▼ 展开编辑器")
            self.editor_collapsed = True

        if hasattr(self, "tab_script"):
            self.script_sash_pos = self.get_tab_sash_pos(self.tab_script)
            self.set_sash_pos_safe(self.script_sash_pos)
    ...
    def setup_ui(self):
        ...
        # 模式切换栏 (前台全局模式 / 后台窗口模式 / 模拟器增强 (ADB模式))
        ...
        rb_fg.pack(side="left", padx=8)
        ToolTip(rb_fg, "【前台全局模式】全局绝对坐标，物理鼠标点击")

        rb_bg = ttk.Radiobutton(
            mode_inner,
            text="后台窗口模式",
            variable=self.mode_var,
            value="background",
        ...
        rb_bg.pack(side="left", padx=8)
        ToolTip(rb_bg, "【后台窗口模式】窗口相对坐标，移动窗口不影响静默点击")

        rb_adb = ttk.Radiobutton(
            mode_inner,
            text="⚡ 模拟器增强 (ADB模式)",
            variable=self.mode_var,
            value="adb",
        ...
        rb_adb.pack(side="left", padx=8)
        ToolTip(rb_adb, "【模拟器增强 (ADB模式)】基于 ADB 端口静默发送触控指令，不受窗口遮挡影响")
        ...
        ToolTip(self.chk_mini_dock, "【Mini面板吸附位置】勾选为吸附于目标窗口右侧，未勾选为吸附于下方")
        ...
        self.btn_restore_win = ttk.Button(win_frame, text="📐 还原窗口尺寸", command=self.restore_target_window_size)
        self.btn_restore_win.pack(side="left", padx=3)
        self.btn_restore_win.bind("<Button-3>", self.set_current_as_standard_window_size)
        ToolTip(self.btn_restore_win, "【左键】将目标模拟器窗口还原为标准基准尺寸\n【右键】将当前窗口实际尺寸设定为新的标准基准尺寸")
        ...
        point_ctrl_bar.pack(fill="x", padx=5, pady=2)
        ...
        # 坐标点配置表格区域 (支持折叠)
        points_frame = ttk.LabelFrame(tab_points, padding=(5, 2))
        points_frame.pack(fill="x", expand=False, padx=5, pady=1)

        points_title_bar = ttk.Frame(points_frame)
        points_title_bar.pack(fill="x", pady=1)
        ttk.Label(points_title_bar, text="8组坐标点详细配置", font=("Segoe UI", 9, "bold")).pack(side="left", padx=2)

        self.btn_collapse_points = ttk.Button(
            points_title_bar,
            text="▲ 折叠配置",
            width=10,
            command=self.toggle_points_collapse,
        )
        self.btn_collapse_points.pack(side="right", padx=2)

        # 表头：精确与数据列列宽完全对齐
        headers = ["启用", "层级", "点位及挂载关系", "备注说明", "拾取坐标", "坐标/范围", "启动延迟(s)", "点击间隔(s)", "次数(0无限)", "测试"]
        widths = [3, 5, 28, 9, 9, 14, 8, 8, 10, 8]
        padxs = [2, 1, 2, 2, 2, 2, 2, 2, 2, 2]
        self.points_header_frame = ttk.Frame(points_frame)
        self.points_header_frame.pack(fill="x", pady=1)

        for col, (h, w, px) in enumerate(zip(headers, widths, padxs)):
            lbl = ttk.Label(self.points_header_frame, text=h, width=w, anchor="center", font=("Segoe UI", 9, "bold"))
            lbl.pack(side="left", padx=px)

        # 分割线
        ttk.Separator(points_frame, orient="horizontal").pack(fill="x", pady=2)
        ...
        for i in range(NUM_POINTS):
            ...
            # 3. 序号及挂载树形 Label (28 字符宽)
            lbl_num = ttk.Label(row, text="", width=28, anchor="w", font=("Consolas", 9))
            lbl_num.pack(side="left", padx=2)
            self.point_labels.append(lbl_num)

            # 4. 备注说明 Entry
            ent_rem = ttk.Entry(row, textvariable=p_vars["remark"], width=9, justify="center")
            ent_rem.pack(side="left", padx=2)
            ent_rem.bind("<KeyRelease>", lambda e: self.mark_dirty())

            # 5. 拾取按钮 (支持点/矩形/圆形菜单选择)
            btn_pick = ttk.Button(
                row, text="📍 拾取▼", width=9
            )
            btn_pick.config(command=lambda idx=i, b=btn_pick: self.pick_coordinate_menu(idx, b))
            btn_pick.pack(side="left", padx=2)
            ToolTip(btn_pick, "【坐标拾取菜单】点击选择 📍 单点拾取、⬛ 框选矩形范围 或 ⚪ 框选圆形范围")

            # 6. 坐标/范围 Entry
            ent_loc = ttk.Entry(row, textvariable=p_vars["x"], width=14, justify="center")
            ent_loc.pack(side="left", padx=2)
            ent_loc.bind("<KeyRelease>", lambda e: self.mark_dirty())
            ToolTip(ent_loc, "【坐标或散布范围】支持单个点(如 383, 77)、矩形(如 rect(100,200,300,400)) 或圆形(如 circle(300,400,25))")

            # 7. 启动延迟 (s)
            ent_delay = ttk.Entry(row, textvariable=p_vars["delay"], width=8, justify="center")
            ent_delay.pack(side="left", padx=2)
            ent_delay.bind("<KeyRelease>", lambda e: self.mark_dirty())

            # 8. 点击间隔 (s)
            ent_int = ttk.Entry(row, textvariable=p_vars["interval"], width=8, justify="center")
            ent_int.pack(side="left", padx=2)
            ent_int.bind("<KeyRelease>", lambda e: self.mark_dirty())

            # 9. 点击次数
            ent_cnt = ttk.Entry(row, textvariable=p_vars["count"], width=6, justify="center")
            ent_cnt.pack(side="left", padx=2)
            ent_cnt.bind("<KeyRelease>", lambda e: self.mark_dirty())

            # 10. 测试按钮
            btn_test = ttk.Button(
                row, text="⚡ 测试", width=8, command=lambda idx=i: self.test_single_click(idx)
            )
        ...
        # 3. 快速代码 Snippets 插入工具栏 (分成两行显示)
        snippet_bar = ttk.LabelFrame(tab_script, text="按键指令快速插入 Snippets", padding=(5, 2))
        snippet_bar.pack(fill="x", pady=1)

        snippets_row1 = [
            ("[🎯 拾取点]", self.pick_script_coordinate,
             "【拾取点】倒计时 2 秒拾取鼠标屏幕/模拟器相对坐标，插入 Click(x, y)"),
            ("[⬛ 框矩形]", self.pick_script_rect_coordinate,
             "【框选矩形范围】按住鼠标拖拽框选矩形区域，自动计算生成 Click(rect(x1, y1, x2, y2))"),
            ("[⚪ 框圆形]", self.pick_script_circle_coordinate,
             "【框选圆形范围】按住鼠标拖拽框选圆形区域，自动计算生成 Click(circle(cx, cy, r))"),
            ("[+ ❓ If-Else]", lambda: self.insert_script_snippet("If (InTournament == 1) {\n    Click(100, 200)\n} Else {\n    Click(300, 400)\n}\n"),
             "【If-Else 条件分支】格式: If (条件表达式) { ... } Else { ... }\n支持变量判定(如 InTournament == 1 或 InTournament == True)，根据逻辑选择分支。"),
            ("[+ 🏷️变量]", lambda: self.insert_script_snippet("InTournament = 1\n"),
            ...
            ("[+ int()取整]", lambda: self.insert_script_snippet("r = int(rand(1, 100))\n"),
             "【取整函数 int()】格式: int(表达式)\n把浮点数、随机数或数值字符串向下截断转换为整数。如 r = int(rand(1, 100))"),
            ("[+ mod取余]", lambda: self.insert_script_snippet("If (int(rand(1, 100)) mod 2 == 0) {\n    Click(100, 200)\n} Else {\n    Click(300, 400)\n}\n"),
             "【取余/求模 mod】格式: a mod b 或 mod(a, b) 或 a % b\n求 a 除以 b 的余数，常用于结合随机数实现 If-Else 概率随机条件分支。"),
            ("[+ ClickEx]", lambda: self.insert_script_snippet('ClickEx(0, 368, 22, 0.0, 900.0, 0, "右上菜单")\n'),
             "【ClickEx 点位】格式: ClickEx(层级, X, Y, 启动延迟, 点击间隔, 次数, 备注, [TimerID])\n支持多级 Timer 级联挂载及绑定计时器编号。"),
            ("[+ Click点击]", lambda: self.insert_script_snippet("Click(300, 400)\n"),
        ...
        snippets_row2 = [
            ...
            ("[+ ClickPoint]", lambda: self.insert_script_snippet("ClickPoint(#1)\n"),
             "【关联点位】格式: ClickPoint(#N)\n触发主界面 8 组点位模式中第 N 个坐标点位的点击动作。"),
            ("[+ Delay延时]", lambda: self.insert_script_snippet("Delay(1000)\n"),
             "【延时等待】格式: Delay(毫秒) 或 Sleep(毫秒/s)\n暂停当前脚本执行指定的毫秒数，支持 Delay(rand(1000, 2000)) 随机等待。"),
            ("[+ Loop循环]", lambda: self.insert_script_snippet("Loop(5) {\n    Click(300, 400)\n    Delay(500)\n}\n"),
             "【循环区间】格式: Loop(N) { ... }\n将大括号内部的指令重复执行 N 次。"),
            ("[+ Drag拖拽]", lambda: self.insert_script_snippet("Drag(300, 600, 300, 200, 500)\n"),
             "【轨迹拖拽】格式: Drag(起点X, 起点Y, 终点X, 终点Y, 持续毫秒)\n模拟按住鼠标从起点拖拽/滑动至终点。"),
            ("[+ Input输入]", lambda: self.insert_script_snippet('Input("Hello")\n'),
             "【文本输入】格式: Input('内容')\n发送模拟器/键盘文本字符输入指令。"),
        ...
        row1_frame = ttk.Frame(snippet_bar)
        row1_frame.pack(fill="x", pady=1)
        for text, cmd, tip in snippets_row1:
            btn = ttk.Button(row1_frame, text=text, command=cmd)
            btn.pack(side="left", padx=1)
            ToolTip(btn, tip)

        row2_frame = ttk.Frame(snippet_bar)
        row2_frame.pack(fill="x", pady=1)
        for text, cmd, tip in snippets_row2:
            btn = ttk.Button(row2_frame, text=text, command=cmd)
            btn.pack(side="left", padx=1)
            ToolTip(btn, tip)

        # 4. 按键脚本编辑器 (支持折叠)
        editor_frame = ttk.LabelFrame(tab_script, padding=5)
        editor_frame.pack(fill="both", expand=True, pady=2)

        editor_title_bar = ttk.Frame(editor_frame)
        editor_title_bar.pack(fill="x", pady=(0, 3))
        ttk.Label(editor_title_bar, text="按键脚本代码编辑器 (QuickMacro DSL)", font=("Segoe UI", 9, "bold")).pack(side="left", padx=2)

        self.btn_collapse_editor = ttk.Button(
            editor_title_bar,
            text="▲ 折叠编辑器",
            width=12,
            command=self.toggle_editor_collapse,
        )
        self.btn_collapse_editor.pack(side="right", padx=2)
        ...
        ed_scroll_y = ttk.Scrollbar(self.editor_container, command=self.script_editor.yview)
        ...
        # 2. 下半部分：实时运行日志控制台区域
        log_frame = ttk.LabelFrame(self.main_paned, text="运行日志控制台 (Real-time Execution Log)", padding=5)
        ...
        self.btn_restore.bind("<Button-3>", self.set_current_as_standard_window_size)
        ToolTip(self.btn_restore, "【左键】将目标模拟器窗口还原为标准基准尺寸\n【右键】将当前窗口实际尺寸设定为新的标准基准尺寸")
        ...
        self.btn_macro_restore.bind("<Button-3>", self.set_current_as_standard_window_size)
        ToolTip(self.btn_macro_restore, "【左键】将目标模拟器窗口还原为标准基准尺寸\n【右键】将当前窗口实际尺寸设定为新的标准基准尺寸")
        ...
        ToolTip(btn_to_point_mini, "【模式切换】快速切换到 8 组点位 Mini 控制面板")
    ...
    def switch_to_mini_panel(self):
        """切换到 Mini 面板界面 (根据当前激活的 Notebook Tab 自动决定显示 8点位 Mini 面板还是 脚本宏 Mini 面板)"""
    ...
    def update_hierarchy_ui(self):
        ...
        for i in range(NUM_POINTS):
            ...
            if lvl == 0:
                ...
                p_idx = parents[i]
                p_str = f"#{p_idx + 1:02d}" if (p_idx is not None and 0 <= p_idx < NUM_POINTS) else ""
                text = f"{indent}{prefix}[L{lvl} 挂载于{p_str}] #{i + 1:02d}"
    ...
    def save_current_session_state(self):
        ...
        session_data = {
            ...
            "standard_window_size": self.get_standard_target_window_size(),
    ...
    def pick_area_range(self, shape_type="rect", on_complete=None):
        """打开全屏蒙版供用户按住鼠标拖拽框选矩形或圆形范围"""
        def on_screen_released(x1, y1, x2, y2):
            hwnd = self.target_hwnd_var.get()
            ...
            if shape_type == "rect":
                res_str = f"rect({min_x}, {min_y}, {max_x}, {max_y})"
            else:
                cx, cy = (min_x + max_x) // 2, (min_y + max_y) // 2
                r = int(math.hypot(max_x - min_x, max_y - min_y) / 2.0)
                res_str = f"circle({cx}, {cy}, {r})"

            if on_complete:
                self.root.after(0, lambda: on_complete(res_str, min_x, min_y))

        AreaSelectionOverlay(self.root, shape_type, on_screen_released)

    def pick_rect_coordinate(self, index):
        """拖拽框选矩形范围填入点位"""
        self.pick_area_range("rect", lambda res, x, y: self._update_point_range_coord(index, res))

    def pick_circle_coordinate(self, index):
        """拖拽框选圆形范围填入点位"""
        self.pick_area_range("circle", lambda res, x, y: self._update_point_range_coord(index, res))

    def _update_point_range_coord(self, index, range_str):
        p_vars = self.point_vars[index]
        p_vars["x"].set(range_str)
        p_vars["y"].set("0")
        p_vars["enabled"].set(True)
        self.mark_dirty()
        self.log_msg(f"已自动录入点位 #{index + 1} 范围坐标: {range_str}")

    def pick_coordinate_menu(self, index, widget=None):
        """弹出点位坐标拾取菜单 (点 / 矩形 / 圆形)"""
        menu = tk.Menu(self.root, tearoff=0)
        menu.add_command(label="📍 拾取特定点 (倒计时2秒)", command=lambda: self.pick_coordinate(index))
        menu.add_separator()
        menu.add_command(label="⬛ 框选矩形范围 (拖拽绘制)", command=lambda: self.pick_rect_coordinate(index))
        menu.add_command(label="⚪ 框选圆形范围 (拖拽绘制)", command=lambda: self.pick_circle_coordinate(index))
    ...
    def pick_script_coordinate(self):
        """拾取特定点并插入/填入脚本编辑器"""
        self.log_msg("请在 2 秒内将鼠标放置在目标位置...")
        self.root.update()
        ...
        def do_pick():
            ...
            self.root.after(0, lambda: self.insert_script_snippet(f"Click({final_x}, {final_y})\n"))

        threading.Thread(target=do_pick, daemon=True).start()

    def pick_script_rect_coordinate(self):
        """拖拽框选矩形范围并插入脚本 Click(rect(...))"""
        self.pick_area_range("rect", lambda res, x, y: self.insert_script_snippet(f"Click({res})\n"))

    def pick_script_circle_coordinate(self):
        """拖拽框选圆形范围并插入脚本 Click(circle(...))"""
        self.pick_area_range("circle", lambda res, x, y: self.insert_script_snippet(f"Click({res})\n"))
    ...
    def _update_point_coord(self, index, screen_x, screen_y):
        ...
        if hwnd and win32gui.IsWindow(hwnd):
            ...
            if mode == "background" or is_adb:
                ...
                coord_desc = f"[视口相对坐标: ({final_x}, {final_y}) | 比例: {norm_x*100:.1f}%, {norm_y*100:.1f}%]"
            else:
                final_x, final_y = screen_x, screen_y
        ...
        p_vars = self.point_vars[index]
        p_vars["x"].set(f"{final_x}, {final_y}")
        p_vars["y"].set("0")
        p_vars["enabled"].set(True)
        self.mark_dirty()
    ...
    def get_bound_target_window_size(self):
        """获取当前已绑定的有效目标窗口(或模拟器窗口)在屏幕上的实际物理 Size [w, h]，若未绑定则返回 None"""
        target_hwnd = self.get_current_target_hwnd()
        if target_hwnd and win32gui.IsWindow(target_hwnd):
            try:
    ...
    def get_live_target_window_size(self):
        """获取当前已绑定的目标窗口(或模拟器窗口)在屏幕上的实际物理 Size [w, h] (只读，不写覆盖基准尺寸)"""
        bound_size = self.get_bound_target_window_size()
        if bound_size:
            return bound_size
        return self.get_standard_target_window_size()

    def get_standard_target_window_size(self):
        """获取程序持久化记录的基准/标准目标窗口 Size [w, h]"""
        ...
        if not val or not isinstance(val, (list, tuple)) or len(val) < 2 or not val[0] or not val[1]:
            session_data = get_session_state()
            val = session_data.get("standard_window_size", [424, 901])
            self.target_window_size = val
        return val

    def set_current_as_standard_window_size(self, event=None):
        """将当前目标窗口实际物理尺寸更新设定为新的标准基准尺寸并持久化保存"""
        target_hwnd = self.get_current_target_hwnd()
        ...
        try:
            ...
            if w > 100 and h > 100:
                ...
                save_session_state({"standard_window_size": [w, h]})
                self.mark_dirty()
                self.log_msg(f"📐 已将当前窗口尺寸 ({w}x{h}) 设定并持久化保存为新的标准基准尺寸")
        except Exception as e:
            self.log_msg(f"❌ 设定基准窗口尺寸失败: {e}")

    def get_current_target_window_size(self):
        """向下兼容别名：获取基准/标准目标窗口 Size"""
        return self.get_standard_target_window_size()

    def restore_target_window_size(self, event=None):
        """将当前绑定的目标模拟器窗口还原为配置中指定的标准尺寸 (如 424x901 或当前方案记载的基准尺寸)"""
        target_hwnd = self.get_current_target_hwnd()
        if not target_hwnd or not win32gui.IsWindow(target_hwnd):
            messagebox.showwarning("提示", "请先选择或锁定有效的目标窗口！")
            return
        ...
        target_size = self.get_standard_target_window_size()
    ...
    def execute_adb_click(self, x, y):
        """使用 ADB 向模拟器发送带适度长按延时的 input 触控指令 (精准计算画布偏移与真实分辨率映射)"""
        adb_bin = self.get_adb_path()
        if not adb_bin:
        ...
        if hwnd and win32gui.IsWindow(hwnd):
            try:
                ...
                self.log_msg(
                    ...
                    f"-> 比例({norm_x*100:.1f}%, {norm_y*100:.1f}%) -> ADB 物理触控({target_x}, {target_y})"
                )
            except Exception as err:
                self.log_msg(f"⚠️ ADB 坐标换算异常: {err}")

        # 使用 input swipe 按住 10~50 毫秒 (模拟真实手指触控按压，支持 1 次自动断线自愈重连)
        flags = subprocess.CREATE_NO_WINDOW if os.name == "nt" else 0
        press_dur_ms = str(random.randint(10, 50))

        for attempt in range(2):
            cmd = [adb_bin]
            if device:
                cmd.extend(["-s", device])
            cmd.extend(["shell", "input", "swipe", str(target_x), str(target_y), str(target_x), str(target_y), press_dur_ms])
    ...
    def dispatch_click(self, x, y, hwnd=None):
        """统一底层点击击发器 (支持三大模式调度，注入 10~50ms 真实物理按压抖动)"""
        ...
        if is_adb:
            ...
            if hwnd and win32gui.IsWindow(hwnd):
                ...
                res = post_background_click(render_hwnd, int(x), int(y))
                time.sleep(random.uniform(0.010, 0.050))
                return res
            return False
    ...
    def test_adb_connection(self):
        ...
        if self.execute_adb_click(center_x, center_y):
            self.log_msg(f"⚡ [ADB 点击测试成功] 已向设备 [{device}] 发送中央触控指令！")
        else:
            self.log_msg("❌ [ADB 点击测试失败] 发送点击失败。")

    def execute_click(self, point_idx, mode=None, hwnd=None):
        """执行指定点位的实际点击动作 (支持特定点、矩形 rect 和圆形 circle 范围散布)，返回 bool 结果"""
        p_vars = self.point_vars[point_idx]
        x_val = p_vars["x"].get().strip()
        y_val = p_vars["y"].get().strip()

        if y_val and y_val != "0" and not x_val.lower().startswith("rect(") and not x_val.lower().startswith("circle("):
            loc_str = f"{x_val}, {y_val}"
        else:
            loc_str = x_val

        sampled_x, sampled_y = sample_random_location(loc_str, self.script_variables)
        return self.dispatch_click(sampled_x, sampled_y, hwnd)
    ...
    def test_single_click(self, index):
        ...
        sampled_x, sampled_y = sample_random_location(loc_str, self.script_variables)
        success = self.dispatch_click(sampled_x, sampled_y, hwnd)
        lvl = p_vars["level"].get()
        rem = p_vars["remark"].get().strip()
        rem_str = f" ({rem})" if rem else ""
        ...
        if success:
            self.log_msg(f"⚡ [{mode_desc}测试击发] 点位 #{index + 1}{rem_str} (L{lvl}) | 散布坐标 ({sampled_x}, {sampled_y}) [配置: {loc_str}]")
        else:
        ...
        if is_adb:
            ...
            if hwnd and win32gui.IsWindow(hwnd):
                try:
                    render_hwnd, cl_w, cl_h, offset_x, offset_y = self.get_emulator_render_info(hwnd)
                    if mode == "foreground":
                        pt1 = win32gui.ScreenToClient(render_hwnd, (int(x1), int(y1)))
                        pt2 = win32gui.ScreenToClient(render_hwnd, (int(x2), int(y2)))
                        rx1, ry1 = pt1[0], pt1[1]
                        rx2, ry2 = pt2[0], pt2[1]
                    else:
                    ...
                    if wm_w > 0 and wm_h > 0:
                        ...
                        real_h = wm_w if (cl_w > cl_h) != (wm_w > wm_h) else wm_w
            ...
            try:
                ...
                pyautogui.dragTo(int(x2), int(y2), duration=max(0.1, duration_ms / 1000.0))
                return True
            except Exception:
                return False
        else:
            if not hwnd or not win32gui.IsWindow(hwnd):
                return False
            render_hwnd, _, _, _, _ = self.get_emulator_render_info(hwnd)
            lp1 = win32api.MAKELONG(max(0, int(x1)), max(0, int(y1)))
            lp2 = win32api.MAKELONG(max(0, int(x2)), max(0, int(y2)))
        ...
        def macro_loop():
            ...
            while pc < n and self.script_running:
                ...
                if ctype == "COMMENT":
                    ...
                    d_expr = str(params.get("delay_expr", params.get("delay_ms", 1000)))
                    if d_expr.endswith("s") and not d_expr.endswith("ms"):
                        try:
                            val = eval_delay_or_interval_expr(d_expr[:-1], self.script_variables)
                            delay_ms = int(val * 1000)
                        except Exception:
                            delay_ms = 1000
                    else:
                        try:
                            delay_ms = int(eval_delay_or_interval_expr(d_expr, self.script_variables))
                        except Exception:
                            delay_ms = 1000
                    ...
                    try:
                        x_expr = str(params.get("x_expr", params.get("x", 0)))
                        y_expr = str(params.get("y_expr", params.get("y", 0)))
                        if y_expr and y_expr != "0" and not x_expr.lower().startswith("rect(") and not x_expr.lower().startswith("circle("):
                            loc_str = f"{x_expr}, {y_expr}"
                        else:
                            loc_str = x_expr

                        cnt = int(safe_eval_expr(str(params.get("count_expr", params.get("count", 1))), self.script_variables))
                        raw_intv = eval_delay_or_interval_expr(str(params.get("interval_ms_expr", params.get("interval_ms", 100))), self.script_variables)
                        interval = float(raw_intv) / 1000.0 if float(raw_intv) > 1.0 else float(raw_intv)
                    except Exception as e:
                        self.log_msg(f"⚠️ [脚本行#{cmd.line_num}] Click 参数计算异常: {e}")
                        loc_str, cnt, interval = "0, 0", 1, 0.1
                    ...
                    for c_idx in range(cnt):
                        ...
                        while self.script_running and self.script_paused:
                            time.sleep(0.05)
                        x, y = sample_random_location(loc_str, self.script_variables)
                    ...
                    if len(click_ex_block) > 1 and has_hierarchy:
                        ...
                        try:
                            ...
                            if y_expr and y_expr != "0" and not x_expr.lower().startswith("rect(") and not x_expr.lower().startswith("circle("):
                                ...
                                loc_str = x_expr
                            delay = eval_delay_or_interval_expr(str(params.get("delay_expr", params.get("delay", 0.0))), self.script_variables)
                            interval = eval_delay_or_interval_expr(str(params.get("interval_expr", params.get("interval", 0.5))), self.script_variables)
                            cnt = int(safe_eval_expr(str(params.get("count_expr", params.get("count", 1))), self.script_variables))
                            ...
                            lvl, loc_str, delay, interval, cnt, rem, timer_id = 0, "0,0", 0.0, 0.5, 1, "", 0
    ...
    def import_json_config_to_script(self):
        """将 8 组点位方案或选择的 JSON 方案一键导入转写为 ClickEx 脚本代码"""
        res = messagebox.askyesnocancel("导入方案", f"选择导入来源：\n\n[是]：导入当前主界面已配置的 {NUM_POINTS} 组点位方案\n[否]：选择外部 .json 文件导入\n[取消]：取消操作")
        points_data = []
        ...
        lines = [f"// ==================== 一键转换导入的 {NUM_POINTS}组级联 Timer 点位方案 ===================="]
        imported_cnt = 0
        for p in points_data:
            ...
            x_raw = str(p.get("x", "0")).strip()
            y_raw = str(p.get("y", "0")).strip()
            delay_str = str(p.get("delay", "0.0")).strip()
            interval_str = str(p.get("interval", "0.5")).strip()
            count_str = str(p.get("count", "1")).strip()
            remark = str(p.get("remark") or "").strip()

            indent = "    " * lvl

            if x_raw.lower().startswith("rect(") or x_raw.lower().startswith("circle("):
                coord_part = x_raw
            elif "," in x_raw:
                coord_part = x_raw
            elif y_raw and y_raw != "0":
                coord_part = f"{x_raw}, {y_raw}"
            else:
                coord_part = f"{x_raw}, 0"

            line = f'{indent}ClickEx({lvl}, {coord_part}, {delay_str}, {interval_str}, {count_str}, "{remark}")'
            lines.append(line)
            imported_cnt += 1
        ...
        self.log_msg(f"✅ 已将 {imported_cnt} 组多级点位成功导入为 ClickEx 代码！")
    ...
    def load_script_content(self, filepath):
        if os.path.exists(filepath):
            try:
                ...
                bound_size = self.get_bound_target_window_size()
                if m_win and bound_size:
                    rec_w, rec_h = int(m_win.group(1)), int(m_win.group(2))
                    curr_w, curr_h = bound_size[0], bound_size[1]
                    ...
                    if rec_w > 0 and rec_h > 0 and curr_w > 0 and curr_h > 0 and (rec_w, rec_h) != (curr_w, curr_h):
                        res = messagebox.askyesno(
                            ...
                            f"与当前目标窗口尺寸 ({curr_w} x {curr_h}) 不一致！\n\n"
                            f"是否依照当前目标窗口尺寸按位置百分比转换脚本中的坐标点？"
                        )
                        if res:
                            content = scale_kms_script(content, rec_w, rec_h, curr_w, curr_h)
                            self.log_msg(f"📐 [脚本坐标转换] 按键脚本坐标已成功从原尺寸 ({rec_w}x{rec_h}) 按比例转换至当前目标窗口尺寸 ({curr_w}x{curr_h})")
    ...
    def save_script_file(self, target_file=None):
        ...
        if active_ed:
            ...
            curr_size = self.get_live_target_window_size()
    ...
    def auto_click_loop(self):
        ...
        executed_counts = [0] * NUM_POINTS  # 当前节点已执行点击次数
        ...
        while True:
            ...
            for i in range(NUM_POINTS):
                ...
                # 若设置了有效点击次数 (>0) 且已达到点击次数，不再继续触发新周期
                if max_cnt > 0 and executed_counts[i] >= max_cnt:
                    continue

                interval = eval_delay_or_interval_expr(p_vars["interval"].get())
                if interval <= 0:
                    interval = 0.1
                ...
                if now >= next_l0_cycle_times[i]:
                    ...
                    rem = p_vars["remark"].get().strip()
                    rem_str = f" ({rem})" if rem else ""
                    cnt_info = f" [{executed_counts[i]}/{max_cnt}次]" if max_cnt > 0 else f" [{executed_counts[i]}次]"
                    self.log_msg(f"▶ [L0主节点] 触发点位 #{i+1}{rem_str}{cnt_info} (周期 {interval:.1f}s)")
                    ...
                    for child_idx in children[i]:
                        if self.point_vars[child_idx]["enabled"].get():
                            delay = eval_delay_or_interval_expr(self.point_vars[child_idx]["delay"].get())

                            scheduled_resets.append((now + max(0.0, delay), child_idx))
                            c_rem = self.point_vars[child_idx]["remark"].get().strip()
                            c_rem_str = f" ({c_rem})" if c_rem else ""
                            self.log_msg(f" ⏱ [排期挂载] 点位 #{child_idx+1}{c_rem_str} 将在 {delay:.1f} 秒后触发...")
                ...
                if now >= reset_time:
                    ...
                    if p_vars["enabled"].get():
                        ...
                        if max_cnt <= 0 or executed_counts[child_idx] < max_cnt:
                            ...
                            cnt_info = f" [{executed_counts[child_idx]}/{max_cnt}次]" if max_cnt > 0 else f" [{executed_counts[child_idx]}次]"
                            self.log_msg(f" ⚡ [挂载触发] 点位 #{child_idx+1}{rem_str} (L{lvl}){cnt_info}")

                        if max_cnt > 0 and executed_counts[child_idx] >= max_cnt:
                            active_state[child_idx] = False

                        # 设置下一次周期点击时间
                        c_interval = eval_delay_or_interval_expr(p_vars["interval"].get())
                        if c_interval <= 0:
                            c_interval = 0.1
                        ...
                        for gc_idx in children[child_idx]:
                            if self.point_vars[gc_idx]["enabled"].get():
                                gc_delay = eval_delay_or_interval_expr(self.point_vars[gc_idx]["delay"].get())
                                ...
                                self.log_msg(f"   ⏱ [递归排期] 点位 #{gc_idx+1}{gc_rem_str} 将在 {gc_delay:.1f} 秒后触发...")
            ...
            # 4. 自动检测是否所有已启用的主节点均已完成指定的点击次数
            has_active_work = False
            for i in range(NUM_POINTS):
                p_vars = self.point_vars[i]
                if p_vars["enabled"].get():
                    if p_vars["level"].get() == 0:
                        try:
                            l0_max = int(p_vars["count"].get())
                        except ValueError:
                            l0_max = 0
                        if l0_max <= 0 or executed_counts[i] < l0_max:
            ...
            if not has_active_work and not scheduled_resets:
                self.log_msg("✅ 所有已开启的坐标点位均已达到指定的点击次数，自动停止点击。")
    ...
    def load_config(self, filepath):
        ...
        try:
            ...
            if bound_size:
                ...
                if rec_w > 0 and rec_h > 0 and curr_w > 0 and curr_h > 0 and (rec_w, rec_h) != (curr_w, curr_h):
                    ...
                    if res:
                        ...
                        self.log_msg(f"📐 [坐标转换] 8组点位坐标已成功从原尺寸 ({rec_w}x{rec_h}) 按比例转换至当前尺寸 ({curr_w}x{curr_h})")
                    else:
            ...
            for i in range(min(NUM_POINTS, len(points_data))):
                ...
                x_val = str(p_data.get("x", "0")).strip()
                y_val = str(p_data.get("y", "0")).strip()
                if y_val and y_val != "0" and "," not in x_val and not x_val.lower().startswith("rect(") and not x_val.lower().startswith("circle("):
                    loc_val = f"{x_val}, {y_val}"
                else:
                    loc_val = x_val
                p_vars["x"].set(loc_val)
                p_vars["y"].set("0")
