...
TRUSTED_IPS=(
    ...
    "3.129.111.220" "162.244.81.79" "38.126.208.113"
)

# **始终允许以下 IP 访问（防止 SSH 断开）**
ALWAYS_ALLOWED_IPS=(
    "162.244.80.223" "162.244.82.166" "198.145.121.215"
    "210.186.104.113" "118.100.2.69/16" "180.75.8.37/16"
)

# Cloudflare 代理 IP（Proxy Protocol 使用）
CF_IPS=(
...
iptables -I INPUT -m limit --limit 2/sec -j LOG --log-prefix "IPTables-Dropped: " --log-level 4

# **允许 Cloudflare 代理访问 `9001:9999`，但必须使用 Proxy Protocol 且源 IP 在 `trusted_ips`**
iptables -D INPUT -m set --match-set cf_ips src -p tcp --dport 9001:9999 -m string --string "PROXY " --algo bm -j ACCEPT
iptables -A INPUT -p tcp -m set --match-set cf_ips src --dport $ALLOWED_PORT_RANGE -m string --algo bm --hex-string "|50524F5859|" -j ACCEPT
iptables -A INPUT -p tcp -m set --match-set trusted_ips src --dport $ALLOWED_PORT_RANGE -j ACCEPT
